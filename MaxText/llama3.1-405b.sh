...
# gcloud storage cp gs://libtpu_internal/mohitkhatwani/ghostlite/2024-11-13-17:34:33-libtpu.so ./libtpu.so

# gcloud storage cp gs://libtpu_internal/mohitkhatwani/ghostlite/2024-11-14-00:05:16-libtpu.so ./libtpu.so

# with luis attempt
# gcloud storage cp gs://libtpu_internal/mohitkhatwani/ghostlite/2024-11-14-21:43:58-libtpu.so ./libtpu.so

# # unsanitized libtpu
# gcloud storage cp gs://libtpu_internal/mohitkhatwani/ghostlite/2024-11-14-22:45:48-libtpu.so ./libtpu.so

# this is after barrier fix
# gcloud storage cp gs://libtpu_internal/mohitkhatwani/ghostlite/2024-11-14-23:22:39-libtpu.so ./libtpu.so

# final libtpu with all the fixes
# gs://libtpu_internal/mohitkhatwani/ghostlite/2024-11-14-23:22:39-libtpu.so
...
# Train
export LIBTPU_INIT_ARGS="--xla_tpu_enable_all_experimental_scheduler_features=true --xla_tpu_host_transfer_overlap_limit=12 \
...
--xla_tpu_enable_data_parallel_all_reduce_opt=true --xla_tpu_enable_async_collective_fusion=true --xla_tpu_enable_async_collective_fusion_fuse_all_gather=ENABLED --xla_tpu_enable_async_collective_fusion_multiple_steps=true \
...
# This is for testing optimizer host offloading

python3 MaxText/$EXECUTABLE MaxText/configs/base.yml model_name=llama3.1-405b \
  steps=20 per_device_batch_size=1 enable_checkpointing=false \
  ici_tensor_parallelism=1 dcn_fsdp_parallelism=1 \
  optimizer_memory_host_offload=true gradient_clipping_threshold=0 \
  remat_policy=custom decoder_layer_input=offload \
  monitor_goodput=false enable_goodput_recording=false \
  max_target_length=8192 \
  base_output_directory=gs://runner-maxtext-logs \
  dataset_type=synthetic allow_split_physical_axes=true profile_cleanly=false profiler=xplane \
  sa_block_q=1024 sa_block_q_dkv=2048 sa_block_q_dq=2048 skip_first_n_steps_for_profiler=5 profiler_steps=2
